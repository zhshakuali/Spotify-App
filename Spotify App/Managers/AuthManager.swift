...
final class AuthManager {
    ...
    struct Constants {
        static let clientID = "11fd8cb9848445e1b40ab5975fc6e6fb"
        static let clientSecret = "082e303adc1a416da07228594a02c7b9"
        static let tokenAPIURL = "https://accounts.spotify.com/api/token"
        static let redirectURL = "https://github.com/zhshakuali/Spotify-App"
    ...
    private var shouldRefreshToken: Bool {
        guard let expirationDate = tokenExpirationToken else { return false }
    ...
    public func exchangeCodeForToken(
        ...
        guard let basicString = data?.base64EncodedString() else {
            print("Failure with base64")
            completion(false)
            return
        }
        request.setValue("Basic \(basicString)", forHTTPHeaderField: "Authorization")
    ...
    public func withValidToken(completion: @escaping(String) -> Void) {
        ...
        if shouldRefreshToken {
            ...
            refreshIfNeeded { [weak self] success in
                if let token = self?.refreshToken, success {
                    completion(token)
                }
            }
        }
